...
export function NotebookView() {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-semibold">{notebook.name}</h1>
            <p className="text-xs text-text-muted">{notebook.sources.filter(s => s.enabled).length} fontes ativas</p>
          </div>
        </div>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-bg-tertiary border border-border rounded-lg text-xs hover:border-accent/50 transition-colors"
          >
            <Brain className="w-3.5 h-3.5 text-accent-light" />
            <span className="text-text-secondary">
              {useStore.getState().settings.defaultModel || 'Selecionar modelo'}
            </span>
      ...
      {/* Tab Navigation */}
      <div className="flex items-center gap-1 px-4 py-2 border-b border-border bg-bg-secondary/30">
        {tabs.map(tab => (
          <button
            key={tab.id}
          ...
          >
            ...
            {tab.label}
            {tab.id === 'sources' && (
              <span className="ml-1 px-1.5 py-0.5 bg-bg-tertiary rounded text-[10px]">
              ...
              <div className="flex items-end gap-2 max-w-4xl mx-auto">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    rows={1}
                    className="w-full px-4 py-3 bg-bg-card border border-border rounded-xl text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent resize-none transition-colors"
                ...
                <button
                  ...
                  className="p-3 bg-accent hover:bg-accent-dark disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl transition-colors"
                >
